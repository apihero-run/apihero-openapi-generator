...
function getSchemaModel(
  ...
  if (schema.type) {
    ...
    model.default = getModelDefault(schema as OpenAPIV3.NonArraySchemaObject, model);
    return model;
  }
...
export const getModelComposition = (
  doc: OpenAPIV3.Document,
  schema: OpenAPIV3.SchemaObject,
  schemas: Array<OpenAPIV3.SchemaObject | OpenAPIV3.ReferenceObject>,
...
export const getRequiredPropertiesFromComposition = (
  ...
  return schemas
    .reduce((properties, s) => {
      if ("$ref" in s) {
        const schema = getRef<OpenAPIV3.SchemaObject>(doc, s);
